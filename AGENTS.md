...
Initial sample data lives in `Data/MockData.swift`. Treat it as a data factory, not persistence. App-wide inventory state currently lives in `AppViewModel.items`; `Space.items` is not the main source of truth. Several flows remain placeholders, including real authentication, natural-language parsing, camera recognition, persistence, push reminders, and some add/save actions.
